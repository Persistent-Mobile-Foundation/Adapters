...
public class JavaSQLResource {
	...
	public Response getUser(@PathParam("userId") String userId) throws SQLException{
		...
		try{
			...
			// NOTE: MySQL returns TYPE_FORWARD_ONLY ResultSet by default.
			// first() is not supported for forward-only cursors, so use next() to check if a row exists.
			if (data.next()) {
	...
	@GET
	@Produces(MediaType.APPLICATION_JSON)
	@OAuthSecurity(scope = "CustomAdapterScope")
